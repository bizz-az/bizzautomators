        ...
        >
          ...
          >
            ...
            <button
              ...
              style={{
                left: 4,
                transform: `translate3d(calc(${progress} * (100% * 0 + var(--span, 0px))), -50%, 0)`,
                marginLeft: `calc(${progress} * (100vw * 0))`,
                translate: `calc(${progress} * (var(--track-span, 0px)))`,
                boxShadow: `0 8px 26px -8px rgba(214,158,74,${0.45 + progress * 0.45})`,
                transition: dragging ? "none" : "translate 420ms cubic-bezier(0.22,1,0.36,1), box-shadow 300ms ease",
              }}
              ref={(el) => {
                const track = trackRef.current;
                if (el && track) {
                  el.style.setProperty("--track-span", `${track.clientWidth - 56 - 8}px`);
                }
              }}
            >
